...
### V3d — automatic BPM detection
Pure logic on the decoded audio buffer (already available via
`getWaveformPeaks`'s decode path): onset detection via energy flux +
autocorrelation of inter-onset intervals, snap to 60–180 BPM range. Offer the
result as a **suggestion** the user confirms — never overwrite a manually set
BPM silently. Acceptance: unit test with a synthesized click track (the WAV
generator in `e2e/editor.spec.ts` shows how to synthesize audio) detecting
within ±2 BPM.
